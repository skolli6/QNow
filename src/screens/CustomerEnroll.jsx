...
import { getVendorById, addToken, subscribeVendorQueue, saveTokenFCM } from '../services/firestoreService'
...
export default function CustomerEnroll() {
  ...
  async function handleGetToken() {
    ...
    try {
      ...
      // FCM (best-effort, don't fail if blocked)
      try {
        const fcmTok = await requestFCMPermission()
        if (fcmTok) await saveTokenFCM(id, fcmTok)
      } catch (e) { /* FCM optional */ }

      // WhatsApp (best-effort)
      sendTokenConfirmation({ name: name.trim(), mobile: m, tokenNumber, vendorName: vendor.name, position: queueLen + 1, waitMins }).catch(() => {})
      ...
      <div className="content pt">
        ...
        {step === 3 && issued && (
          <>
            ...
            <div className="card">
              <div className="card-title">📌 You'll get WhatsApp alerts when:</div>
              <div style={{marginTop:10,fontSize:'.85rem',color:'var(--text2)',lineHeight:1.85}}>
                💬 Only <strong>3 people ahead</strong> of you<br/>
                💬 <strong>15 minutes</strong> before your turn<br/>
                💬 <strong>It's your turn</strong> — go to the counter!<br/>
                ⏰ Need more time? Tap <strong>Extend Slot</strong> on the tracking screen
              </div>
